...
use pnet::{packet::{arp::{ArpHardwareTypes, ArpPacket, MutableArpPacket}, ethernet::{EtherTypes, EthernetPacket, MutableEthernetPacket}, Packet}, util::MacAddr};
...
fn construct_proxyarp_frames(arplog: ArpLog, sender_mac: MacAddr) -> ([u8; 42], [u8; 42]){
    ...
    let mut ethernet_frame1 = MutableEthernetPacket::new(&mut ethernet_buffer1).expect("Packet Creation Failed");
    let mut ethernet_frame2 = MutableEthernetPacket::new(&mut ethernet_buffer2).expect("Packet Creation Failed");
